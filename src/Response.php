...
class Response
{
    ...
    public function __construct($body, $status = 200)
    {
        ...
        $this->headers = [
            ...
            'Server' => 'PHPServer/1.0.0 (Ghaith8bit)'
        ];
    }
    ...
    {
        $lines = [];
        $lines[] = "HTTP/1.1 {$this->status} " . self::$statusMessage[($this->status)] ?? 'Unknown Status';
    ...
    {
        $body = 'Error ' . $statusCode . ': ' . self::$statusMessage[$statusCode] ?? 'Unknown Error';
